...
function Sidebar({sidebarOpen,s}) {
  ...
  {/* First 3 buttons: shown if s is true OR sidebarOpen is true */}
  {(s || sidebarOpen) && (
    <> 
      <button onClick={()=> navigate("/")}><img src={HomeLogo} alt="Home" />{ "Home"}</button>
       <button><img src={Shorts} alt="Shorts" />{ "Shorts"}</button>
      <button><img src={Subscription} alt="Subscriptions" />{ "Subscriptions"}</button>
    </>
  )}

  {/* "You" button always shown if (s || sidebarOpen), but section content only when sidebarOpen */}
  {(s || sidebarOpen) && (
    <div className={`sidecontainer ${sidebarOpen ? "expanded" : ""}`}>
      <button style={sidebarOpen ? { fontSize: "18px" } : {}}>
        {sidebarOpen ? "You >" : (<><img src={Human} alt="You" /><br />You</>)}
      </button>
